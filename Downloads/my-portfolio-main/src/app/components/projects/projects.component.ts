...
@Component({
  ...
  template: `
    <section id="projects">
      <div class="container">
        ...
        <!-- Featured (shown only for 'all' or 'angular') -->
        <ng-container *ngIf="activeFilter === 'all' || activeFilter === 'angular'">
          <div class="featured-project" *ngFor="let p of featuredProjects; let i = index" [class.reverse]="i % 2 !== 0">
            <div class="fp-visual">
              <div class="fp-screen">
                <div class="fp-bar">
            ...
            <div class="fp-info">
              <span class="fp-tag">{{ isArabic ? 'مشروع مميز' : 'Featured Project' }}</span>
              <h3 class="fp-title">{{ p.title }}</h3>
              <div class="fp-desc-box"><p>{{ isArabic ? p.descriptionAr : p.description }}</p></div>
              <div class="fp-techs">
        ...
        <!-- Other projects grid -->
        <h3 class="other-title" *ngIf="filteredOtherProjects.length > 0">
          {{ isArabic ? 'مشاريع أخرى' : 'More Projects' }}
        </h3>
        <div class="other-grid" *ngIf="filteredOtherProjects.length > 0">
          <div class="project-card" *ngFor="let p of filteredOtherProjects; let i = index" [style.animation-delay]="(i * 0.08) + 's'">
            ...
            <div class="card-body">
              ...
              <p class="card-desc">{{ isArabic ? p.descriptionAr : p.description }}</p>
        ...
        <!-- Empty state -->
        <div class="empty-state" *ngIf="filteredOtherProjects.length === 0 && activeFilter !== 'all' && activeFilter !== 'angular'">
          <p>{{ isArabic ? 'لا توجد مشاريع في هذا القسم حالياً' : 'No projects in this category yet' }}</p>
        </div>
  ...
  styles: [`
    ...
    /* Featured */
    .featured-project { display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center; margin-bottom: 100px; animation: fadeInUp 0.5s ease both; }
    .featured-project.reverse { direction: rtl; }
    .featured-project.reverse > * { direction: ltr; }

    .fp-screen { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; box-shadow: 0 24px 80px rgba(0,0,0,0.4); transition: transform 0.4s var(--ease-out-expo); }
    .fp-screen:hover { transform: scale(1.02) rotate(-0.5deg); }
    ...
    .fp-desc-box { background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 18px; margin-bottom: 20px; }
    .fp-desc-box p { color: var(--text-muted); line-height: 1.7; font-size: 14px; }
    ...
    .fp-link:hover { border-color: var(--accent); color: var(--accent); transform: translateY(-2px); }
    .fp-link.live:hover { border-color: var(--accent-3); color: var(--accent-3); }
    ...
    .card-links { display: flex; gap: 10px; color: var(--text-dim); }
    .card-links a { transition: color 0.2s; }
    .card-links a:hover { color: var(--accent); }
    .card-desc { color: var(--text-muted); font-size: 13px; line-height: 1.6; margin-bottom: 14px; }
    .card-techs { display: flex; gap: 10px; flex-wrap: wrap; }
    .card-techs span { font-family: var(--font-mono); font-size: 10px; color: var(--text-dim); }
...
export class ProjectsComponent {
  ...
  filters = [
    ...
    { key: 'angular', label: 'Angular',  labelAr: 'Angular' },
    { key: 'html',    label: 'HTML/CSS', labelAr: 'HTML/CSS' },
  ];
  ...
  getCount(key: string): number {
    if (key === 'all') return this.featuredProjects.length + this.otherProjects.length;
    const featured = this.featuredProjects.filter(p => p.category === key).length;
    const other = this.otherProjects.filter(p => p.category === key).length;
    return featured + other;
  }

  get filteredOtherProjects() {
    if (this.activeFilter === 'all') return this.otherProjects;
    return this.otherProjects.filter(p => p.category === this.activeFilter);
  }
  ...
  featuredProjects = [
    {
      ...
      descriptionAr: 'منصة رياضية نسائية عربية كاملة مبنية بـ Angular 17. تتضمن فلترة تصنيفات الرياضة، خلاصات المقالات، مصادقة المستخدم، وتخطيط RTL متجاوب بواجهة مستخدم حديثة.',
    ...
    },
    {
      ...
      descriptionAr: 'موقع وكالة احترافي لفريق تطوير ويب. يتضمن تبديل الوضع الداكن/الفاتح، تبديل اللغة عربي/إنجليزي، أقسام التسعير، عرض الفريق، الأسئلة الشائعة، ونموذج تواصل.',
  ...
  otherProjects = [
    {
      ...
      descriptionAr: 'موقع مطعم بقائمة طعام كاملة، معرض صور، ملفات الطهاة، وأقسام العروض الخاصة.',
    ...
    },
    {
      ...
      descriptionAr: 'صفحة هبوط أنيقة لمحل شوكولاتة مع سلايدر، قصة تقليد عائلي، وجماليات فاخرة.',
    ...
    },
    {
      ...
      descriptionAr: 'متجر أزياء إلكتروني بكتالوج المنتجات، عربة التسوق، ومصادقة المستخدم.',
    ...
    },
    {
      ...
      descriptionAr: 'مكوّن إشعارات تفاعلي بحالات مقروء/غير مقروء وتحديثات واجهة مستخدم في الوقت الفعلي.',
      techs: ['HTML', 'CSS', 'JavaScript'],
    ...
    },
    {
      ...
      descriptionAr: 'صفحة هبوط نظيفة لتطبيق إدارة الحافظة مع أزرار تحميل.',
      techs: ['HTML', 'CSS'],
